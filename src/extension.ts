...
export function activate(context: vscode.ExtensionContext) {
	...
	console.log('Congratulations, your extension "maestro-helper" is now active!');

	// The command has been defined in the package.json file
	// Now provide the implementation of the command with registerCommand
	// The commandId parameter must match the command field in package.json
	const disposable = vscode.commands.registerCommand('maestro-helper.helloWorld', () => {
	...
	const snippetsPath = path.join(context.extensionPath, 'snippets', 'maestro.code-snippets.json');
	let snippets: any = {};
	...
	const completionProvider = vscode.languages.registerCompletionItemProvider("maestro", {
		provideCompletionItems(document, position, token, context) {
			...
			if (!isAtLineStart && !isAfterDash && !isAfterDashWithText && currentPrefix.length === 0) {
				return undefined;
			}
			...
			for (const [snippetName, snippetData] of Object.entries(snippets)) {
				...
				if (snippet.prefix && Array.isArray(snippet.prefix)) {
					for (const prefix of snippet.prefix) {
						...
						if (currentPrefix.length > 0 && prefixLower.startsWith(currentPrefix)) {
							...
							completionItem.sortText = `${prefix.length.toString().padStart(3, '0')}_${prefix}`;
	...
	// Register for both "maestro" and "yaml" languages to ensure it works
	const createHoverProvider = (isYaml: boolean = false) => ({
		provideHover(document: vscode.TextDocument, position: vscode.Position) {
			// For YAML provider, only work on maestro files
			if (isYaml) {
				const fileName = document.fileName.toLowerCase();
				const isMaestroFile = fileName.endsWith('.flow') ||
			...
			const charAtCursor = position.character < line.length ? line[position.character] : '';
			...
			let match;
			const matches: Array<{key: string, start: number, end: number, colonPos: number}> = [];
			...
			for (const m of matches) {
				...
				if (position.character >= m.start && position.character <= m.colonPos + 2) {
			...
			if (charAtCursor === ':' || (position.character > 0 && line[position.character - 1] === ':')) {
	...
	const hoverProviderMaestro = vscode.languages.registerHoverProvider("maestro", createHoverProvider(false));
	const hoverProviderYaml = vscode.languages.registerHoverProvider("yaml", createHoverProvider(true));
	...
	context.subscriptions.push(disposable, hoverProviderMaestro, hoverProviderYaml, completionProvider);
}
